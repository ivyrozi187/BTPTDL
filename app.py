...
@st.cache_resource # Dùng st.cache_resource để lưu mô hình (TF-IDF, Cosine Sim)
def load_and_preprocess_data():
    """Tải và tiền xử lý dữ liệu, trả về các mô hình và DataFrame."""
    try:
        ...
        st.error(f"LỖI TẢI HOẶC XỬ LÝ DỮ LIỆU: {e}. Vui lòng kiểm tra các file CSV.")
        st.stop()


def initialize_user_data(df_movies):
    """Khởi tạo hoặc tải dữ liệu người dùng vào Session State."""
    if 'all_users_df' not in st.session_state:
        try:
        ...
        st.session_state['all_users_df'] = df_users
    
    # Trả về DataFrame hiện tại, bao gồm cả người dùng mới đăng ký
    return st.session_state['all_users_df']
...
def register_new_user_form(df_movies):
    ...
    df_users = initialize_user_data(df_movies)
    ...
    with st.form("register_form"):
        ...
        if submitted:
            # 1. Kiểm tra đầu vào
            if not username or username in df_users['Tên người dùng'].values:
                st.error("❌ Tên người dùng không hợp lệ hoặc đã tồn tại.")
                return
            ...
            st.session_state['all_users_df'] = pd.concat([df_users, new_user_df], ignore_index=True)
...
def login_form():
    ...
    df_users = st.session_state['all_users_df']
...
def get_recommendations(username, df_movies, num_recommendations=10):
    ...
    # ... (Logic hàm get_recommendations giữ nguyên, sử dụng df_users từ session state) ...
    ...
    if menu_choice == 'Đề xuất theo Tên Phim':
        ...
        if st.button("Tìm Đề Xuất", key="find_sim"):
            ...
            if not result.empty:
                ...
                st.dataframe(result[['Tên phim', 'Thể loại phim', 'Độ phổ biến']], use_container_width=True)
...
if __name__ == '__main__':
    ...
    df_movies, cosine_sim = load_and_preprocess_data()
    
    # Khởi tạo hoặc tải dữ liệu người dùng (Bắt buộc phải sau load_and_preprocess_data)
    df_users = initialize_user_data(df_movies)
